...
The following diagrams illustrate the architecture and build flow of `Alchemy`.

### Build System Architecture

<div class="mermaid">
flowchart TB
    subgraph ALCHEMY["🔧 ALCHEMY BUILD SYSTEM"]
        direction TB
        
        subgraph Entry["Phase 1: Entry Point"]
            A["alchemake.py<br/>Build Wrapper"] --> B["main.mk<br/>Core Engine"]
            B --> C["Module Discovery<br/>Scan atom.mk"]
        end
        
        subgraph Setup["Phase 2: Configuration"]
            D["Target Setup<br/>TARGET_xxx"] 
            E["Toolchain Setup<br/>Compiler/Linker"]
            F["Module Classes<br/>BUILD_xxx"]
        end
        ...
        D & E & F --> G["Build Rules Generation"]
        
        subgraph Output["Phase 3: Build Output"]
            H["Compilation<br/>Objects"] --> I["Staging<br/>Libraries/Binaries"]
            I --> J["Final Tree<br/>Stripped Image"]
        end
...
flowchart LR
    subgraph Classes["MODULE CLASSES"]
        ...
        subgraph Internal["🔨 INTERNAL - Source-based"]
            I1["EXECUTABLE"]
            I2["STATIC_LIBRARY"]
            I3["SHARED_LIBRARY"]
            I4["LIBRARY<br/>(static+shared)"]
            I5["PREBUILT"]
        end
        
        subgraph External["📦 EXTERNAL - Build Systems"]
            E1["AUTOTOOLS<br/>configure/make"]
            E2["CMAKE<br/>CMakeLists.txt"]
            E3["MESON<br/>meson.build"]
            E4["QMAKE<br/>Qt projects"]
            E5["PYTHON_PACKAGE<br/>setup.py"]
            E6["CUSTOM<br/>Custom scripts"]
            E7["META_PACKAGE<br/>Dependencies only"]
            E8["LINUX_MODULE<br/>Kernel modules"]
        end
    end
</div>
...
flowchart TB
    subgraph Pipeline["🔄 BUILD PIPELINE"]
        ...
        P1["📥 Source Files<br/>atom.mk definitions"]
        P2["⚙️ Configuration<br/>kconfig / global.config"]
        P3["🔨 Compilation<br/>Objects generation"]
        P4["📦 Staging<br/>Libraries + Headers"]
        P5["✂️ Final<br/>Stripped binaries"]
        P6["💾 Image<br/>tar/cpio/ext4/plf"]
...
flowchart TB
    subgraph Output["📂 TARGET_OUT"]
        ...
        subgraph Build["build/"]
            B1["module-1/<br/>Objects, .o files"]
            B2["module-2/<br/>Objects, .o files"]
            B3["...<br/>Other modules"]
        end
        
        subgraph Staging["staging/"]
            S1["usr/bin/<br/>Executables"]
            S2["usr/lib/<br/>Libraries (.so, .a)"]
            S3["usr/include/<br/>Headers"]
        end
        
        subgraph Final["final/"]
            F1["bin/, lib/<br/>Stripped binaries"]
            F2["etc/<br/>Configuration"]
            F3["boot/<br/>Kernel, DTB"]
        end
        
        subgraph Extra["Additional"]
            X1["symbols/<br/>Debug symbols"]
            X2["gcov/<br/>Coverage data"]
        end
    end
</div>
...
This section shows how to build simple C and C++ programs with `Alchemy`.
...
<pre>
my_project/
├── alchemy/                    # Alchemy build system (clone/symlink)
├── config/                     # Configuration directory
│   └── product.mk              # Product configuration
├── hello_c/                    # C example
│   ├── atom.mk                 # Module definition
│   └── src/
...
**File: `hello_c/atom.mk`**
<pre>
LOCAL_PATH := $(call my-dir)

include $(CLEAR_VARS)

LOCAL_MODULE := helloworld-c
LOCAL_DESCRIPTION := Hello World example in C
...
**File: `hello_cpp/src/helloworld.cpp`**
<pre>
#include &lt;iostream&gt;
#include &lt;string&gt;
...
**File: `hello_cpp/atom.mk`**
<pre>
LOCAL_PATH := $(call my-dir)

include $(CLEAR_VARS)

LOCAL_MODULE := helloworld-cpp
LOCAL_DESCRIPTION := Hello World example in C++
LOCAL_CATEGORY_PATH := examples
...
LOCAL_CXXFLAGS := -Wall -Werror -std=c++11
...
**File: `config/product.mk`**
<pre>
# Target configuration for native Linux build
TARGET_OS := linux
...
export TARGET_PRODUCT="helloworld"
export TARGET_PRODUCT_VARIANT="native"
export TARGET_CONFIG_DIR="${TOP_DIR}/config"
export TARGET_OUT="${TOP_DIR}/out"
export ALCHEMY_USE_COLORS=1

# Build using alchemake
${TOP_DIR}/alchemy/scripts/alchemake.py \
    -f ${TOP_DIR}/alchemy/main.mk \
    "$@"
</pre>
...
./build.sh helloworld-cpp
</pre>

4. **Run the executables:**
<pre>
./out/staging/usr/bin/helloworld-c
./out/staging/usr/bin/helloworld-cpp
</pre>

5. **Clean a module:**
<pre>
./build.sh helloworld-c-clean
./build.sh helloworld-cpp-dirclean
</pre>

6. **Configure modules (optional):**
<pre>
./build.sh config
</pre>

### Advanced: HelloWorld with Library Dependency

**File: `hello_lib/atom.mk`** - A shared library
<pre>
LOCAL_PATH := $(call my-dir)
...
LOCAL_DESCRIPTION := Hello shared library
LOCAL_CATEGORY_PATH := examples/libs
...
**File: `hello_app/atom.mk`** - Application using the library
<pre>
LOCAL_PATH := $(call my-dir)

include $(CLEAR_VARS)

LOCAL_MODULE := hello-app
LOCAL_DESCRIPTION := Application using libhello
LOCAL_CATEGORY_PATH := examples
...
| Command | Description |
|---------|-------------|
| `./build.sh all` | Build all configured modules |
| `./build.sh <module>` | Build specific module with dependencies |
| `./build.sh <module>-clean` | Clean module (keep sources for external) |
| `./build.sh <module>-dirclean` | Remove entire build directory |
| `./build.sh config` | Configure modules (kconfig UI) |
| `./build.sh config-update` | Update configuration silently |
| `./build.sh help` | Show Alchemy help |
| `./build.sh final` | Generate final stripped tree |
| `./build.sh image` | Generate filesystem image |
| `./build.sh V=1 <module>` | Verbose build (show commands) |
| `./build.sh -j8 all` | Parallel build with 8 jobs |

* * *

## Building the Linux Kernel

`Alchemy` provides built-in support for compiling the Linux kernel as a module.
The kernel is treated as an external module with class `LINUX`.

### Kernel Module Architecture

<div class="mermaid">
flowchart TB
    subgraph LinuxBuild["🐧 LINUX KERNEL BUILD"]
        direction TB
        
        subgraph Config["Configuration"]
            C1["defconfig"] --> C2["menuconfig"]
            C2 --> C3[".config"]
        end
        
        subgraph Build["Build Process"]
            B1["vmlinux"] --> B2["zImage/bzImage"]
            B2 --> B3["Device Trees"]
            B3 --> B4["Modules .ko"]
        end
...
LOCAL_CATEGORY_PATH := system/kernel

# Path to kernel source
LOCAL_LINUX_SRC := $(LOCAL_PATH)/kernel

# Default configuration file
LOCAL_LINUX_DEFCONFIG := my_board_defconfig

# Additional configuration fragments (optional)
LOCAL_LINUX_CONFIG_FILES := \
    $(LOCAL_PATH)/config/debug.cfg \
    $(LOCAL_PATH)/config/drivers.cfg

# Device tree files to build
LOCAL_LINUX_DEVICE_TREE_NAMES := \
    my_board.dtb \
    my_board_v2.dtb
...
The following `TARGET_XXX` variables control kernel compilation:

| Variable | Description |
|----------|-------------|
| `TARGET_LINUX_DIR` | Path to kernel source tree |
| `TARGET_LINUX_CROSS` | Cross-compiler prefix for kernel |
| `TARGET_LINUX_IMAGE` | Image type: `zImage`, `bzImage`, `Image` |
| `TARGET_LINUX_DEFCONFIG` | Default kernel configuration |
| `TARGET_LINUX_DEVICE_TREE_NAMES` | List of DTB files to install |
| `TARGET_LINUX_GENERATE_UIMAGE` | Generate U-Boot image (0/1) |
| `TARGET_LINUX_LINK_CPIO_IMAGE` | Link initramfs in kernel (0/1) |
| `TARGET_LINUX_MAKE_BUILD_ARGS` | Extra make arguments |
...
For external kernel modules (drivers), use the `LINUX_MODULE` class:

**File: `my_driver/atom.mk`**
<pre>
LOCAL_PATH := $(call my-dir)

include $(CLEAR_VARS)

LOCAL_MODULE := my-driver
LOCAL_DESCRIPTION := My custom kernel driver
LOCAL_CATEGORY_PATH := system/drivers
...
### Complete Example: HelloWorld Kernel Driver

This example shows how to create a complete "Hello World" kernel module with Alchemy.

#### Project Structure

<pre>
hello_driver/
├── atom.mk              # Alchemy module definition
├── Kbuild               # Kernel build file (optional)
└── src/
    └── hello_module.c   # Driver source code
</pre>

#### Driver Source Code

**File: `hello_driver/src/hello_module.c`**
<pre>
/*
 * Hello World Linux Kernel Module
 * Built with Alchemy build system
 */
...
**File: `hello_driver/atom.mk`**
<pre>
LOCAL_PATH := $(call my-dir)

include $(CLEAR_VARS)

LOCAL_MODULE := hello-driver
LOCAL_DESCRIPTION := Hello World kernel module example
LOCAL_CATEGORY_PATH := examples/drivers

# Source files for the kernel module
LOCAL_SRC_FILES := \
    src/hello_module.c

# Dependency on Linux kernel (must be built first)
LOCAL_LIBRARIES := linux

# Optional: Additional compiler flags for kernel module
LOCAL_CFLAGS := -DDEBUG

include $(BUILD_LINUX_MODULE)
</pre>

#### Optional Kbuild File

If you need more control over the kernel build, create a `Kbuild` file:

**File: `hello_driver/Kbuild`**
<pre>
# Kernel module name (without .ko extension)
obj-m := hello_module.o

# For multi-file modules:
# hello_module-objs := file1.o file2.o file3.o
...
./build.sh hello-driver

# The module will be in:
# out/staging/lib/modules/&lt;kernel-version&gt;/extra/hello_module.ko

# Clean the module
./build.sh hello-driver-clean

# Full clean
./build.sh hello-driver-dirclean
</pre>
...
<pre>
# Load the module
sudo insmod /lib/modules/$(uname -r)/extra/hello_module.ko

# Load with parameter
sudo insmod hello_module.ko name="Alchemy"

# Check kernel messages
dmesg | tail
# Output: Hello, World! Module loaded.
...
dmesg | tail
# Output: Goodbye, World! Module unloaded.
</pre>

#### Advanced: Character Device Driver

A more complete example with a character device:

**File: `chardev_driver/src/chardev.c`**
<pre>
#include &lt;linux/init.h&gt;
#include &lt;linux/module.h&gt;
#include &lt;linux/kernel.h&gt;
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Your Name");
MODULE_DESCRIPTION("Hello CharDev Driver");
                         ...
                         size_t len, loff_t *offset)
{
    ...
    msg_buffer[len] = '\0';
    msg_len = len;
    printk(KERN_INFO "hello_chardev: Received %zu bytes\n", len);
    return len;
}
...
static int __init chardev_init(void)
{
    ...
    strcpy(msg_buffer, "Hello from kernel!\n");
    msg_len = strlen(msg_buffer);
    printk(KERN_INFO "hello_chardev: Registered with major %d\n", major_num);
...
static void __exit chardev_exit(void)
{
    ...
    printk(KERN_INFO "hello_chardev: Unregistered\n");
}
...
**File: `chardev_driver/atom.mk`**
<pre>
LOCAL_PATH := $(call my-dir)

include $(CLEAR_VARS)

LOCAL_MODULE := hello-chardev
LOCAL_DESCRIPTION := Hello World character device driver
LOCAL_CATEGORY_PATH := examples/drivers

LOCAL_SRC_FILES := src/chardev.c
LOCAL_LIBRARIES := linux

include $(BUILD_LINUX_MODULE)
</pre>

### Product Configuration for Kernel Build

**File: `config/product.mk`**
<pre>
# Kernel configuration
TARGET_LINUX_DIR := $(TOP_DIR)/kernel
TARGET_LINUX_CROSS := /opt/toolchain/bin/arm-linux-gnueabihf-
TARGET_LINUX_IMAGE := zImage
TARGET_LINUX_GENERATE_UIMAGE := 1

# Device trees to install in boot/
TARGET_LINUX_DEVICE_TREE_NAMES := \
    my_board.dtb

# Additional kernel make arguments
TARGET_LINUX_MAKE_BUILD_ARGS := \
    INSTALL_DTBS_PATH="$(TARGET_OUT_STAGING)/boot"
</pre>
...
`Alchemy` supports building applications and libraries for Android targets using
either the Android NDK or a custom Android build system.
...
flowchart TB
    subgraph AndroidBuild["🤖 ANDROID BUILD"]
        ...
        subgraph NDK["Android NDK Mode"]
            N1["NDK Toolchain"] --> N2["Bionic libc"]
            N2 --> N3["Native Libraries"]
            N3 --> N4["JNI .so"]
        end
        
        subgraph Custom["Custom Android"]
            A1["Alchemy-android"] --> A2["Custom Toolchain"]
            A2 --> A3["System Libraries"]
        end
...
**File: `config/product.mk`** for Android NDK:
<pre>
# Android target configuration
TARGET_OS := linux
...
TARGET_ANDROID_NDK := /opt/android-ndk-r21e
TARGET_ANDROID_SDK := /opt/android-sdk
TARGET_ANDROID_APILEVEL := 21
TARGET_ANDROID_MINAPILEVEL := 16

# C++ STL choice: gnustl, libc++, stlport
TARGET_ANDROID_STL := libc++

# Use shared STL for multiple shared libraries
...
export TARGET_PRODUCT="myapp"
export TARGET_PRODUCT_VARIANT="android"
export TARGET_CONFIG_DIR="${TOP_DIR}/config-android"
export TARGET_OUT="${TOP_DIR}/out-android"
...
# NDK path
export TARGET_ANDROID_NDK=/opt/android-ndk-r21e
export TARGET_ANDROID_APILEVEL=21

ALCHEMY_USE_COLORS=1 ${TOP_DIR}/alchemy/scripts/alchemake.py \
    -f ${TOP_DIR}/alchemy/main.mk \
    "$@"
</pre>

### Android JNI Library Example

**File: `jni_module/atom.mk`**
<pre>
LOCAL_PATH := $(call my-dir)
...
LOCAL_DESCRIPTION := JNI native library for Android
LOCAL_CATEGORY_PATH := android/jni
...
|----------|-------------|--------|
| `TARGET_ANDROID_NDK` | Path to Android NDK | Auto-detect |
| `TARGET_ANDROID_SDK` | Path to Android SDK | Auto-detect |
| `TARGET_ANDROID_APILEVEL` | Target API level | 17 |
| `TARGET_ANDROID_MINAPILEVEL` | Minimum API level | Same as APILEVEL |
| `TARGET_ANDROID_TOOLCHAIN` | Toolchain name | Auto |
| `TARGET_ANDROID_STL` | STL implementation | gnustl |
| `TARGET_ANDROID_SHARED_STL` | Use shared STL (0/1) | 0 |

### Multi-Architecture Build

To build for multiple Android architectures, create separate build configurations:

**File: `build-android-arm.sh`**
<pre>
export TARGET_ARCH=arm
export TARGET_PRODUCT_VARIANT="android-arm"
export TARGET_OUT="${TOP_DIR}/out-android-arm"
# ... rest of configuration
</pre>

**File: `build-android-arm64.sh`**
<pre>
export TARGET_ARCH=aarch64
export TARGET_PRODUCT_VARIANT="android-arm64"
export TARGET_OUT="${TOP_DIR}/out-android-arm64"
# ... rest of configuration
</pre>
...
./build-android.sh mynativelib

# Generate final tree with stripped binaries
./build-android.sh final

# Copy output to Android project
cp -r out-android/staging/usr/lib/*.so android_app/app/src/main/jniLibs/armeabi-v7a/
</pre>

### Android vs Native Conditional Compilation

In your `atom.mk`, you can have conditional code based on target:
...
LOCAL_MODULE := mylib
LOCAL_DESCRIPTION := Cross-platform library

LOCAL_SRC_FILES := \
    src/common.c

# Android-specific sources and flags
ifeq ("$(TARGET_OS_FLAVOUR)","android")
    LOCAL_SRC_FILES += src/android_impl.c
    LOCAL_CFLAGS += -DANDROID_BUILD
    LOCAL_LDLIBS += -llog
else
    LOCAL_SRC_FILES += src/linux_impl.c
endif
...
This class of modules is used to build and install python packages based on
